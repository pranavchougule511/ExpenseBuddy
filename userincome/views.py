...
def income_category_summary(request):
    ...
    incomeData = UserIncome.objects.filter(owner=request.user,date__gte=six_months_ago, date__lte=todays_date)
    finalrep = {}

    def get_category(income):
        return income.source
    category_list = list(set(map(get_category, incomeData)))

    def get_income_category_amount(source):
        amount = 0
        filtered_by_category = UserIncome.objects.filter(source=source)
